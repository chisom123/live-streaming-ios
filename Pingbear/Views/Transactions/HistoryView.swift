...
final class HistoryViewModel: ObservableObject {
    ...
    func load() {
        ...
        db.collection("content_transactions")
            ...
            .getDocuments { snap, _ in
                defer { group.leave() }
                all.append(contentsOf: snap?.documents.compactMap { ContentTransaction(id: $0.documentID, data: $0.data()) } ?? [])
            }
        ...
        group.notify(queue: .main) { [weak self] in
            ...
            Task { await self.enrich(txs: all.sorted { $0.createdAt > $1.createdAt }) }
        }
    }
    ...
    private func enrich(txs: [ContentTransaction]) async {
        ...
        for tx in txs {
            ...
            if let otherId = tx.otherUserId(currentUserId: currentUserId) {
                if let cached = profileCache[otherId] { profile = cached }
                else {
...
struct HistoryView: View {
    ...
    var body: some View {
        ...
        .onAppear { Analytics.shared.trackScreen(name: "history_view"); vm.load() }
        .sheet(item: $selectedTransaction) { enriched in
            HistoryDetailView(enriched: enriched, currentUserId: currentUserId, onDismiss: { selectedTransaction = nil })
        }
    }
    ...
    var body: some View {
        HStack(spacing: 14) {
            ...
            VStack(alignment: .trailing, spacing: 4) {
                ...
                Text(tx.createdAt.timeAgoShort).font(.system(size: 11)).foregroundColor(AppTheme.secondaryText)
            }
            .padding(.trailing, 16)
        }
...
struct HistoryDetailView: View {
                    ...
                    VStack(spacing: 20) {
                        ...
                        HStack(spacing: 16) {
                            ...
                            VStack(alignment: .leading, spacing: 3) {
                                Text(otherName).font(.system(size: 16, weight: .bold)).foregroundColor(AppTheme.primaryText)
                                if let username = enriched.otherProfile?.username {
                                    Text("@\(username)").font(.system(size: 13)).foregroundColor(AppTheme.secondaryText)
                                }
                            }
                            Spacer()
                            VStack(alignment: .trailing, spacing: 3) {
                                if isCreator && tx.status == .completed {
                                    Text("+$\(String(format: "%.2f", tx.creatorPayout))")
                                        .font(.system(size: 18, weight: .black)).foregroundColor(AppTheme.green)
                                    Text("earned").font(.system(size: 11)).foregroundColor(AppTheme.secondaryText)
                                } else if !isCreator {
                                    Text("-$\(String(format: "%.2f", tx.price))")
                                        .font(.system(size: 18, weight: .black)).foregroundColor(AppTheme.primaryText)
                                    Text("paid").font(.system(size: 11)).foregroundColor(AppTheme.secondaryText)
                                }
                            }
                        }
                        ...
                        VStack(spacing: 0) {
                            ...
                            Divider().background(AppTheme.divider).padding(.leading, 16)
                            detailRow(label: "Date", value: tx.createdAt.formatted(date: .long, time: .shortened))
                            if let completedAt = tx.completedAt {
                                Divider().background(AppTheme.divider).padding(.leading, 16)
                                detailRow(label: "Completed", value: completedAt.formatted(date: .long, time: .shortened))
                            }
                        }
                        .background(AppTheme.cardBackground).cornerRadius(12).padding(.horizontal, 20)

                        if tx.status == .completed && tx.rating == nil && !isCreator {
                            RatingCard(transactionId: tx.id, onRated: { }).padding(.horizontal, 20)
                        }
                    }
            ...
            .toolbar {
                ToolbarItem(placement: .navigationBarLeading) {
                    Button(action: onDismiss) {
                        Image(systemName: "xmark").font(.system(size: 16, weight: .bold)).foregroundColor(AppTheme.iconColor)
                    }
                }
            }
        }
    ...
    private func detailRow(label: String, value: String) -> some View {
        HStack(alignment: .top, spacing: 12) {
            Text(label).font(.system(size: 13)).foregroundColor(AppTheme.secondaryText).frame(width: 90, alignment: .leading)
            Text(value).font(.system(size: 13, weight: .semibold)).foregroundColor(AppTheme.primaryText).multilineTextAlignment(.leading)
            Spacer()
        }
