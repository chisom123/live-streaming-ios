    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 24) {
                    ZStack {
                        Circle().fill(AppTheme.green.opacity(0.15)).frame(width: 100, height: 100)
                        Image(systemName: "gift.fill").font(.system(size: 44)).foregroundColor(AppTheme.green)
                    }
                    VStack(spacing: 10) {
                    ...
                    Text("+$5").font(.system(size: 28, weight: .bold)).foregroundColor(.white)
                        .padding(EdgeInsets(top: 10, leading: 28, bottom: 10, trailing: 28))
                        .background(AppTheme.green).cornerRadius(200)
                }
                .frame(maxWidth: .infinity).padding(.horizontal, 20).padding(.vertical, 50)
                .background(AppTheme.cardBackground).cornerRadius(16).padding(.horizontal, 20)
